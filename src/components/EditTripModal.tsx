...
import { X, Calendar, Globe, Trash2, Save, Plus, MapPin, Info } from "lucide-react";
import { updateTrip, deleteTrip, Trip, Expense } from "@/lib/db";
...
export default function EditTripModal({ isOpen, onClose, trip }: EditTripModalProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    notes: trip.notes || "",
    categoryBudgets: {
      Food: trip.categoryBudgets?.Food?.toString() || "",
      Transport: trip.categoryBudgets?.Transport?.toString() || "",
      Accommodation: trip.categoryBudgets?.Accommodation?.toString() || "",
      Activities: trip.categoryBudgets?.Activities?.toString() || "",
      Other: trip.categoryBudgets?.Other?.toString() || ""
    }
  });
